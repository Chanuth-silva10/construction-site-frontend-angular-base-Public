...
export class EditOrderComponent implements OnInit {
    ...
    this.getAllSites();
      ...
      this.orderService.getOrderById(id).subscribe(
        (data:any) => {
          ...
          console.log(data);
          this.site?.setValue(data.site._id);
          this.supplier?.setValue(data.supplier._id);
          this.deliveryAddress?.setValue(data.deliveryAddress);
          this.deliveryAddress?.setValue(data.deliveryAddress);
          this.orderNotes?.setValue(data.orderNotes);
          // this.orderItems?.push(...data.items);
          this.commentsArr=data.comments;
          this.total=data.total;
          console.log(this.addOrderForm.value)

          let current_datetime = new Date(data.expectedDeliveryDate);
          let formatted_date = current_datetime.getFullYear() + "-" + (current_datetime.getMonth()+1>9?current_datetime.getMonth()+1:"0"+current_datetime.getMonth()+1) + "-" +(current_datetime.getDate()>9?current_datetime.getDate():"0"+current_datetime.getDate());
          this.expectedDeliveryDate?.setValue(formatted_date);


          let i =0;
          data.items.map(item=>{
            this.orderItems.at(i).get('qty')?.setValue(item.qty);
            this.orderItems.at(i).get('itemId')?.setValue(item.itemId._id);
            this.orderItems.at(i).get('price')?.setValue(item.price);
  ...
  submit(): void {
    if (this.addOrderForm.valid) {
      ...
      const order: any = {
        ...
        approvals:this.order.approvals
      }

      console.log(order);

      // Call save/update function from Item service
        this.orderService.updateOrder(this.order._id!, order).subscribe(
          () => {
            this.success();
          },
          () => {
  ...
  private success(): void {
    this.feedback.success(`Order updated successfully!`);
